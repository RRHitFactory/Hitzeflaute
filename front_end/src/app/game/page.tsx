    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <main className="max-w-full mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="text-xs text-gray-500 bg-white p-2 rounded border mb-4 max-w-[400px]">
          <div><span className="font-semibold">Cookie Player ID:</span> {cookiePlayerId ?? 'null'}</div>
          <div><span className="font-semibold">Current Player:</span> {currentPlayerObj ? JSON.stringify(currentPlayerObj) : 'null'}</div>
          <div><span className="font-semibold">Controls Enabled:</span> {controlsEnabled.toString()}</div>
        </div>
