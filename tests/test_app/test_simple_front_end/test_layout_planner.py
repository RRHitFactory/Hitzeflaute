...
class TestLayoutPlanner(TestCase):
    def test_sockets_are_deterministic(self) -> None:
        Plan = tuple[dict[AssetId, SocketAddress], dict[TransmissionId, tuple[SocketAddress, SocketAddress]]]

        game_state = GameStateMaker().make()
        func = LayoutPlanner.get_socket_addresses_for_assets_and_transmission
